...
test("development appointment inbox is protected and reports persistence state", async ({ page }) => {
  await page.goto("/appointments");
  await expect(page.getByRole("heading", { name: "Public Request Inbox" })).toBeVisible();
  await expect(page.getByText(/Supabase is not configured in this local environment/)).toBeVisible();
...
test("branch page shows confirmed details and Daet exception", async ({ page }) => {
  ...
  await expect(page.getByRole("main").getByRole("link", { name: "+63 920 735 1379" })).toBeVisible();
