...
test("public home uses authentic clinic, attendant, logo, and diode assets", async ({ page }) => {
  ...
  await expect(page.getByRole("img", { name: "ESCLARE 4D diode laser device visual in a prepared treatment room" })).toBeVisible();
...
test("treatment gallery filters verified media and preserves booking selection", async ({ page }) => {
  await page.goto("/home");
  await expect(page.getByRole("heading", { name: "A closer look, with clinical honesty." })).toBeVisible();
  await expect(page.getByRole("img", { name: "Pico laser facial treatment visual with protective eyewear in a private treatment room" })).toBeVisible();

  await page.getByRole("tab", { name: "MCCM PDRN Glow" }).click();
  await expect(page.getByRole("img", { name: "MCCM PDRN professional-use vial and packaging on a neutral display" })).toBeVisible();
  await page.getByRole("button", { name: "Open MCCM PDRN Glow image: MCCM PDRN professional formula" }).click();
