  ...
  return (
    <form action={formAction} className="grid min-w-60 gap-2">
      ...
      <select
        id={`status-${requestId}`}
        name="status"
        className="min-h-9 border border-neutral-300 bg-white px-2 text-sm text-neutral-800"
      >
        {options.map((status) => (
      ...
      <input
        ...
        minLength={3}
        maxLength={300}
        placeholder="Reason or contact note"
        className="min-h-9 border border-neutral-300 px-2 text-sm outline-none focus:border-[#6F263D]"
      />
      <button
        type="submit"
        disabled={isPending}
        className="inline-flex min-h-9 items-center justify-center gap-2 bg-[#5B1830] px-3 text-xs font-semibold text-white disabled:opacity-60"
      >
