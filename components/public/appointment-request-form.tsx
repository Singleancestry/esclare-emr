...
export function AppointmentRequestForm({
  ...
  const confirmationBranch = state.submittedBranchCode
    ...
    <div className="grid gap-0 overflow-hidden rounded-lg border border-[#D8C9B4] bg-white shadow-[0_24px_70px_rgba(55,28,37,0.1)] lg:grid-cols-[1fr_0.72fr]">
      ...
      >
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm leading-6 text-[#6B6264]">
            Only your full name is required. Add visit preferences when you are ready.
          </p>
        </div>
        <div className="grid gap-5">
          ...
          <label className="grid gap-2 text-xs font-bold uppercase tracking-[0.05em] text-[#3F1724]">
            Treatment{" "}
            <span className="font-normal normal-case tracking-normal text-[#77716A]">Optional</span>
            <select
              name="treatmentSlug"
              value={treatmentSlug}
          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <label className="grid gap-2 text-xs font-bold uppercase tracking-[0.05em] text-[#3F1724]">
              ...
              <span className="relative">
                <CalendarDays
                  className="pointer-events-none absolute left-3 top-3.5 text-[#7A655D]"
                  size={17}
                />
                <input
                  name="preferredDate"
          ...
          {error && (
            <p
              id="booking-error"
              role="alert"
              className="rounded-lg border-l-2 border-[#B42D43] bg-[#FFF2F3] px-3 py-2 text-sm text-[#8B1F32]"
            >
              {error}
            </p>
          )}
          ...
          <button
            type="submit"
            disabled={isPending}
            className="luxury-button w-full disabled:cursor-wait disabled:opacity-70 sm:w-fit"
          >
            {isPending ? (
              <>
                <LoaderCircle className="animate-spin" size={17} /> Sending request
              </>
            ) : (
              "Prepare request"
            )}
      ...
      <aside className="bg-[#EEE6DA] p-5 sm:p-9 lg:p-11">
        <p className="public-eyebrow">Step 2</p>
        <h2 className="mt-3 text-3xl text-[#481827]">Confirm with {confirmationBranch.name}</h2>
        <p className="mt-3 text-sm leading-6 text-[#625D58]">
          {confirmationBranch.schedule}.
          {confirmationBranch.lastClient ? ` ${confirmationBranch.lastClient}` : ""}
        </p>
        {prepared ? (
          <div
            className="mt-6 border border-[#D3C2AC] bg-white p-4"
            role="status"
            aria-live="polite"
          >
            <p className="flex items-center gap-2 text-sm font-semibold text-[#316847]">
              <CheckCircle2 size={17} />{" "}
              {state.status === "saved" ? "Request saved" : "Request ready"}
            </p>
            {state.reference && (
              <p className="mt-2 text-xs font-bold uppercase text-[#6F263D]">
            ...
            <button
              type="button"
              onClick={copyMessage}
              className="mt-4 inline-flex items-center gap-2 text-xs font-bold uppercase text-[#5B1830]"
            >
              <Copy size={14} /> {copied ? "Copied" : "Copy message"}
            </button>
          </div>
        ) : (
          <p className="mt-6 text-sm leading-6 text-[#625D58]">
            Enter your full name to prepare the message you can send through an official channel.
          </p>
        )}
        {copyError && (
          <p role="alert" className="mt-3 text-xs text-[#8B1F32]">
            The message could not be copied. You can still open Messenger, call, or send an SMS.
          </p>
        )}
        <div className="mt-6 grid gap-3">
          ...
          <a
            href={`sms:${confirmationBranch.phoneHref}${prepared ? `?body=${encodeURIComponent(`${message}${state.reference ? ` Reference: ${state.reference}.` : ""}`)}` : ""}`}
            className="luxury-button-outline justify-start bg-white"
          >
